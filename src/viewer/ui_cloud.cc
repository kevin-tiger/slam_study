...
std::vector<Vector4f> UiCloud::intensity_color_table_pcl_;

UiCloud::UiCloud(CloudPtr cloud) { SetCloud(cloud, Sophus::SE3d()); }

void UiCloud::SetCloud(CloudPtr cloud, const Sophus::SE3d& pose) {
    ...
    Sophus::SE3f pose_f = pose.cast<float>();
    std::for_each(std::execution::par_unseq, idx.begin(), idx.end(), [&](const int& id) {
        ...
        xyz_data_[id] = Vector3f(pt_world.x(), pt_world.y(), pt_world.z());
        color_data_pcl_[id] = IntensityToRgbPCL(pt.intensity);
        color_data_gray_[id] = Vector4f(0.5, 0.5, 0.5, 0.2);
        color_data_height_[id] = IntensityToRgbPCL(pt.z * 10);
        color_data_intensity_[id] =
            Vector4f(pt.intensity / 255.0 * 3.0, pt.intensity / 255.0 * 3.0, pt.intensity / 255.0 * 3.0, 0.2);
    });
...
void UiCloud::BuildIntensityTable() {
    ...
    auto make_color = [](int r, int g, int b) -> Vector4f { return Vector4f(r / 255.0f, g / 255.0f, b / 255.0f, 0.2f); };
    for (int i = 0; i < 256; i++) {
