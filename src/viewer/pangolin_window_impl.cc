...
bool PangolinWindowImpl::Init() {
    ...
    // 轨迹
    traj_lidarloc_ui_.reset(new UiTrajectory(Vector3f(1.0, 0.0, 0.0)));      // 红色
    traj_gps_ui_.reset(new UiTrajectory(Vector3f(1.0, 1.0, 51.0 / 255.0)));  // 黄色

    current_scan_.reset(new PointCloudType);
    current_scan_ui_.reset(new UiCloud);
...
bool PangolinWindowImpl::UpdateGlobalMap() {
    ...
    for (const auto &cp : cloud_global_map_) {
        ...
        std::shared_ptr<UiCloud> ui_cloud(new UiCloud);
        ui_cloud->SetCloud(cp.second, Sophus::SE3d());
        ui_cloud->SetRenderColor(UiCloud::UseColor::GRAY_COLOR);
...
bool PangolinWindowImpl::UpdateCurrentScan() {
    ...
    if (current_scan_ != nullptr && !current_scan_->empty() && current_scan_need_update_) {
        if (current_scan_ui_) {
            current_scan_ui_->SetRenderColor(UiCloud::UseColor::HEIGHT_COLOR);
            scans_.emplace_back(current_scan_ui_);
        }

        current_scan_ui_ = std::make_shared<UiCloud>();
...
bool PangolinWindowImpl::UpdateState() {
    ...
    Vector3d pos = pose_.translation().eval();
    Vector3d vel_baselink = pose_.so3().inverse() * vel_;
