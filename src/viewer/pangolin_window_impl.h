...
public:
    ...
    Sophus::SE3d current_pose_;                 // 当前scan对应的pose

    // 地图点云
    std::map<Vector2i, CloudPtr, math::less_vec<2>> cloud_global_map_;

    /// gps
    Sophus::SE3d gps_pose_;
...
private:
    ...
    /// cloud rendering
    UiCar car_{Vector3f(0.2, 0.2, 0.8)};                                      // 白色车
    std::map<Vector2i, std::shared_ptr<UiCloud>, math::less_vec<2>> cloud_map_ui_;  // 用来渲染的点云地图
    std::shared_ptr<UiCloud> current_scan_ui_;                             // current scan
    std::deque<std::shared_ptr<UiCloud>> scans_;                           // current scan 保留的队列

    /// ui绘制中使用的一些中间变量
    Sophus::SE3d T_map_odom_for_lio_traj_ui_;      // 用于显示lio的轨迹
    Sophus::SE3d T_map_baselink_for_lio_traj_ui_;  // 用于显示lio的轨迹

    //  trajectory
    std::shared_ptr<UiTrajectory> traj_lidarloc_ui_ = nullptr;
    std::shared_ptr<UiTrajectory> traj_gps_ui_ = nullptr;
