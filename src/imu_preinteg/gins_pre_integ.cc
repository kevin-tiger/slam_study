...
void GinsPreInteg::SetOptions(GinsPreInteg::Options options) {
    ...
    prior_info_.block<6, 6>(9, 9) = Matrix<double, 6, 6> ::Identity() * 1e6;
...
void GinsPreInteg::AddOdom(const Odom& odom) {
    last_odom_ = odom;
    last_odom_set_ = true;
}
...
void GinsPreInteg::Optimize() 
{
    ...
    Vector3d vel_odom = Vector3d::Zero();
    if (last_odom_set_) {
        // velocity obs
        double velo_l =
        ...
        vel_odom = Vector3d(average_vel, 0.0, 0.0);
