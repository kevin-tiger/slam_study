 ...
 */
template <typename T>
inline bool esti_plane_dynamic(Eigen::Matrix<T, 4, 1>& abcd, const std::vector<Vector3d>& point, const double& threshold) {
    if (point.size() < 3) {
        cout << "the number of points should not be less than 3, given " << point.size() << endl;
        return false;
    }
    // Marked by xiaotao:
...
inline bool PoseInterp(double query_time, C&& data, FT&& take_time_func, FP&& take_pose_func, Sophus::SE3d& result,
                       T& best_match, float time_th = 0.5) {
    if (data.empty()) {
        cout << "cannot interp because data is empty. " << endl;
        return false;
    }
...
inline double exp(Vector3d& result, const Vector3d& vec, const double& scale = 1) {
...
inline Sophus::SO3d exp(const Vector3d& vec, const double& scale = 1) {
    double norm2 = vec.squaredNorm();
    std::pair<double, double> cos_sinc = cos_sinc_sqrt(scale * scale * norm2);
    double mult = cos_sinc.second * scale;
    Vector3d result = mult * vec;
    return Sophus::SO3d(Quaterniond(cos_sinc.first, result[0], result[1], result[2]));
}
...
inline Eigen::Matrix<double, 2, 3> PseudoInverse(const Eigen::Matrix<double, 3, 2>& X) {
    ...
    Vector2d sv = svd.singularValues();
 ...
 * @return
 */
inline Eigen::Matrix<double, 3, 3> A_matrix(const Vector3d& v) {
    ...
    if (norm < 1e-5) {
        ...
        res = Eigen::Matrix<double, 3, 3>::Identity() + (1 - std::cos(norm)) / squaredNorm * Sophus::SO3d::hat(v) +
 ...
 */
template <typename T>
bool PoseInterp(double query_time, const std::map<double, T>& data, const std::function<Sophus::SE3d(const T&)>& take_pose_func,
                Sophus::SE3d& result, T& best_match) {
    if (data.empty()) {
    ...
    if (query_time > data.rbegin()->first) {
        cout << "query time is later than last, " << std::setprecision(18) << ", query: " << query_time
                  << ", end time: " << data.rbegin()->first << endl;
    ...
    Sophus::SE3d pose_first = take_pose_func(match_iter->second);
    Sophus::SE3d pose_next = take_pose_func(match_iter_n->second);
    result = {pose_first.unit_quaternion().slerp(s, pose_next.unit_quaternion()),
              pose_first.translation() * (1 - s) + pose_next.translation() * s};
