...
LoopClosure::LoopClosure(const std::string& config_yaml) 
:
yaml_(config_yaml) 
{
...
bool LoopClosure::Init() 
{
    ...
    {
        cout << "cannot load keyframes" << endl;
        return false;
    }
...
void LoopClosure::DetectLoopCandidates() 
{
    ...
    cout << "detecting loop candidates from pose in stage 1" << endl;
    ...
    for (auto iter_first = keyframes_.begin(); iter_first != keyframes_.end(); ++iter_first) {
        ...
        for (auto iter_second = iter_first; iter_second != keyframes_.end(); ++iter_second) {
            ...
            Vector3d dt = kf_first->opti_pose_1_.translation() - kf_second->opti_pose_1_.translation();
    ...
    cout << "detected candidates: " << loop_candiates_.size() << endl;
}
...
void LoopClosure::ComputeLoopCandidates()
{
    ...
    std::for_each(loop_candiates_.begin(), loop_candiates_.end(),
                  [this](LoopCandidate& c) { ComputeForCandidate(c); });
    // 保存成功的候选
    ...
    cout << "success: " << succ_candidates.size() << "/" << loop_candiates_.size() << endl;
...
{
    cout << "aligning " << c.idx1_ << " with " << c.idx2_ << endl;
    ...
    auto build_submap = [this](int given_id, bool build_in_world) -> CloudPtr {
        ...
        for (int idx = -submap_idx_range; idx < submap_idx_range; idx += 4) {
            ...
            // 转到世界系下
            Sophus::SE3d Twb = kf->opti_pose_1_;
    ...
    Matrix<float, 4, 4> Tw2 = kf2->opti_pose_1_.matrix().cast<float>();
    ...
    Matrix<double, 4, 4> T = Tw2.cast<double>();
    Quaterniond q(T.block<3, 3>(0, 0));
    q.normalize();
    Vector3d t = T.block<3, 1>(0, 3);
    c.Tij_ = kf1->opti_pose_1_.inverse() * Sophus::SE3d(q, t);
