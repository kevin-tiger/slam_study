...
public:
    ...
    struct VoxelData 
    {
        ...
        std::vector<Vector3d> pts_;       // 内部点，多于一定数量之后再估计均值和协方差
        Vector3d mu_ = Vector3d::Zero();     // 均值
        Matrix3d sigma_ = Matrix3d::Zero();  // 协方差
        Matrix3d info_ = Matrix3d::Zero();   // 协方差之逆
    ...
    {
        options_.inv_voxel_size_ = 1.0 / options_.voxel_size_;
        cout << "IncNdt3d, options_.inv_voxel_size_ = " << options_.inv_voxel_size_ << "m" << endl;
        GenerateNearbyGrids();
    }
    /// 获取一些统计信息
     ...
     * @param HTVr
     */
    void ComputeResidualAndJacobians(const Sophus::SE3d& pose, Matrix<double, 18, 18>& HTVH, Matrix<double, 18, 1>& HTVr);
    /**
     * 根据估计的NDT建立edges
     * @param v
     * @param edges
...
private:
    ...
    std::unordered_map<KeyType, std::list<KeyAndData>::iterator, math::hash_vec<3>> grids_;  // 栅格数据，存储真实数据的迭代器
